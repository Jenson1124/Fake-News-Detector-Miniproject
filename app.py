...
STOP_WORDS = set(stopwords.words("english")) | {
    ...
    'two', 'three', 'new', 'year', 'say', 'told', 'added', 'according',
    'pti', 'ani', 'ians'
}
...
.header-badge  { display:inline-block; background:#f0f4ff; color:#3b5bdb; font-size:0.72rem; font-weight:600;
                 letter-spacing:0.8px; text-transform:uppercase; padding:3px 10px; border-radius:20px;
                 margin-bottom:0.8rem; font-family:'JetBrains Mono',monospace; }

.result-real { background:linear-gradient(135deg,#f0fdf4 0%,#dcfce7 100%); border:1.5px solid #86efac;
               border-left:5px solid #16a34a; border-radius:12px; padding:1.4rem 1.8rem; margin:1rem 0; }
.result-fake { background:linear-gradient(135deg,#fff7f7 0%,#fee2e2 100%); border:1.5px solid #fca5a5;
               border-left:5px solid #dc2626; border-radius:12px; padding:1.4rem 1.8rem; margin:1rem 0; }
...
.prob-pill { flex:1; text-align:center; padding:0.6rem 1rem; border-radius:8px; border:1.5px solid #e2e8f0; background:#f8fafc; }
.prob-pill-label { font-size:0.72rem; text-transform:uppercase; letter-spacing:0.8px; font-weight:600; color:#94a3b8; font-family:'JetBrains Mono',monospace; }
...
.lime-section { background:#f8fafc; border:1.5px solid #e2e8f0; border-radius:12px; padding:1.2rem 1.4rem; margin:1.2rem 0; }
...
.news-meta     { font-size:0.78rem; color:#94a3b8; margin:0 0 0.5rem; font-family:'JetBrains Mono',monospace; }
...
.sidebar-metric-val   { font-size:1.5rem; font-weight:700; color:#0f1923; font-family:'JetBrains Mono',monospace; }

div[data-testid="stTabs"] button { font-family:'Inter',sans-serif; font-weight:500; font-size:0.9rem; }
...
div[data-testid="stTextArea"] textarea {
    border-radius:8px; font-family:'Inter',sans-serif; font-size:0.93rem; border:1.5px solid #e2e8f0; }
</style>
...
def get_lime(text: str):
    try:
        ...
        exp = explainer.explain_instance(text, predict_fn, num_features=20, num_samples=500)
...
# ── NewsAPI ───────────────────────────────────────────────────────────────────
@st.cache_data(show_spinner=False, ttl=600)  # cache 10 min
def fetch_live_news(query: str, sources: str = "", language: str = "en", page_size: int = 12):
...
def fetch_top_headlines(country: str = "in", category: str = "general", page_size: int = 12):
    """Fetch top headlines — for Indian news use country='in'."""
    params = {
        "country":  country,
        "category": category,
        "pageSize": page_size,
...
def get_source_meta(source_id: str, source_name: str):
    """Return (flag, display_name) for a news source."""
    if source_id and source_id in SOURCE_META:
        return SOURCE_META[source_id]
    # Guess country from name
    indian_keywords = ['hindu', 'ndtv', 'india', 'times of india', 'hindustan', 'express',
                       'economic times', 'mint', 'deccan', 'tribune', 'wire']
    name_lower = (source_name or "").lower()
    if any(k in name_lower for k in indian_keywords):
        return ("🇮🇳", source_name)
    return ("📰", source_name or "Unknown")

def analyse_articles(articles: list) -> list:
    """Run prediction on a list of NewsAPI articles."""
    results = []
    for art in articles:
        ...
        if not text or text == "[Removed]" or title == "[Removed]":
            continue

        source_id   = (art.get("source") or {}).get("id",   "") or ""
        source_name = (art.get("source") or {}).get("name", "") or ""
        flag, display_name = get_source_meta(source_id, source_name)
...
with st.sidebar:
    ...
    if model is None:
        ...
        if ds:
            ...
            has_india = ds.get("india_articles", 0) > 0
            info_parts = []
            if ds.get("us_articles"):
                info_parts.append(f"🇺🇸 {ds['us_articles']:,} US articles")
            if ds.get("india_articles"):
                info_parts.append(f"🇮🇳 {ds['india_articles']:,} Indian articles")
            info_html = "<br>".join(info_parts) if info_parts else f"{ds.get('total_articles',0):,} articles"
            st.markdown(
                f"<div style='font-size:0.82rem;color:#334155;background:#f1f5f9;"
                f"padding:8px 10px;border-radius:6px;line-height:1.8'>"
                f"{info_html}</div>",
                unsafe_allow_html=True
            )
            if not has_india:
                st.info("💡 Add Indian dataset for better Indian news accuracy — see download_indian_dataset.py")

        st.markdown(" ")
        st.markdown("**Features**")
        st.markdown(
            "<div style='font-size:0.85rem;color:#334155;background:#f1f5f9;"
...
with tab_text:
    ...
    if t.strip():
        input_text = t.strip()
    st.markdown(
        "<small style='color:#94a3b8'>💡 Tip: longer articles give more accurate results "
        "— TF-IDF works best with more text.</small>",
        unsafe_allow_html=True
    )
...
with tab_url:
    ...
    if st.button("Fetch article →", disabled=not bool(url)):
        ...
        if fetched:
            ...
            st.error("Could not fetch article — try pasting the text directly.")
...
with tab_live:
    ...
    st.markdown(
        ...
        "Fetches real headlines from NewsAPI and runs them through the detector. "
        "Results show which articles the model flags — always verify with the original source."
        "</span></div>",
        unsafe_allow_html=True
    )
    ...
    with live_col2:
        live_count = st.selectbox("Articles to fetch", [6, 10, 15, 20], index=1)
    ...
    if live_mode == "🌐 Search any topic":
        ...
        with search_col2:
            lang_opt = st.selectbox("Language", ["English", "Hindi"], label_visibility="collapsed")
        lang_code = "en" if lang_opt == "English" else "hi"
    else:
        live_query = ""
    ...
    if fetch_btn:
        with st.spinner("Fetching live news and running analysis…"):
        ...
        if not articles:
            st.error("No articles returned. Check your internet connection or try a different query.")
        else:
            results = analyse_articles(articles)
            if not results:
                st.warning("Articles were fetched but could not be analysed.")
            else:
                # Summary row
                ...
                s1.metric("Articles analysed", len(results))
                s2.metric("✅ Likely Real",    real_count)
                s3.metric("❌ Likely Fake",    fake_count)
                ...
                for r in filtered:
                    ...
                    bar_color = "#16a34a" if is_real else "#dc2626"
                    if uncertain:
                        bar_color = "#d97706"
                ...
                st.markdown(
                    "<small style='color:#94a3b8'>⚠️ This tool analyses <b>writing style</b>, "
                ...
                if st.button("📥 Export results as CSV"):
                    ...
                    st.download_button(
                        "⬇️ Download CSV",
                        data=csv,
                        file_name=f"live_news_analysis_{datetime.date.today()}.csv",
                        mime="text/csv",
                    )

# ══════════════════════════════════════════════════════════════════════════════
# ANALYSE (manual input tabs)
# ══════════════════════════════════════════════════════════════════════════════
st.markdown(" ")
...
if analyse:
    ...
    <div class="{card_cls}">
        ...
        <div class="result-sub">
            Based on writing style, vocabulary patterns, and linguistic features
        </div>
        <div class="conf-wrap">
    ...
    if use_lime and word_weights:
        ...
        <div class="lime-section">
            ...
            <div class="lime-legend">
                <span style="background:#bbf7d0;color:#14532d;padding:1px 6px;border-radius:3px;font-weight:600;font-size:0.78rem">GREEN</span>
                supports <b>Real</b> &nbsp;·&nbsp;
                <span style="background:#fecaca;color:#7f1d1d;padding:1px 6px;border-radius:3px;font-weight:600;font-size:0.78rem">RED</span>
                supports <b>Fake</b> &nbsp;·&nbsp;
    ...
    if len(input_text.split()) < 30:
        st.markdown(
            "<small style='color:#94a3b8'>ℹ️ Short text — confidence may be lower. "
            "Paste a full article for a more reliable result.</small>",
            unsafe_allow_html=True
        )
